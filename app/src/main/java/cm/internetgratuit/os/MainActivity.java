...
public class MainActivity extends Activity {
    ...
    private void openSidebarFromBack(){String js="(function(){var selectors=['button[aria-label*=menu i]','button[title*=menu i]','.menu-toggle','.sidebar-toggle','.navbar-toggler','[data-toggle=sidebar]','[data-bs-toggle=offcanvas]'];for(var i=0;i<selectors.length;i++){var b=document.querySelector(selectors[i]);if(b){b.click();return 'clicked';}}var q=document.querySelector('.sidebar,.app-sidebar,aside[class*=sidebar],nav[class*=sidebar]');if(q){q.classList.add('open');q.classList.add('active');q.style.display='block';q.style.visibility='visible';q.style.transform='translateX(0)';return 'opened';}return 'none';})();";webView.evaluateJavascript(js,value->{if(value==null||value.contains("none"))Toast.makeText(MainActivity.this,"Vous êtes déjà sur l'accueil",Toast.LENGTH_SHORT).show();});}
    ...
    @Override public void onBackPressed(){if(webView==null)return;if(webView.canGoBack()){webView.goBack();return;}String current=webView.getUrl()==null?"":webView.getUrl();String home=AppConfig.route(serverUrl,"/sav-dashboard");if(home!=null&&!home.isEmpty()&&!current.startsWith(home)){openRoute("/sav-dashboard");return;}openSidebarFromBack();}
}
